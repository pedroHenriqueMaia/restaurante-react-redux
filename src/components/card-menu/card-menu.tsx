...
const CardMenu: React.FC<IEntradaCardMenu> = ({ selectedItem }) => {
  ...
  const cartItem = cartItems.find((item: IResponseMenu) => item.id === selectedItem.id);
  ...
  return (
    ...
    <div className="card" style={{ width: '25rem', height:'18rem' }}>
      <div className="card-body">
        <h5 className="card-title">{selectedItem.name} ({selectedItem.category})</h5>
        <p className="card-text">{selectedItem.description}</p>
        <div className="row mt-5">
          <p className="col-5">{selectedItem.origin}</p>
          <p className="col-5">${selectedItem.price.toLocaleString('pt-BR', { minimumFractionDigits:2 })}</p>
        ...
        {quantity === 0 ? (
          ...
          <div className="d-flex justify-content-center align-items-center">
            <button className="btn btn-outline-primary me-2" onClick={handleRemoveFromCart}>
              -
            </button>
            <span>{quantity}</span>
