...
const cartSlice = createSlice({
  ...
  reducers: {
    addItemToCart: (state, action: PayloadAction<IResponseMenu>) => {
      ...
      if (existingItem) {
        existingItem.quantity === undefined ? existingItem.quantity = 2 : existingItem.quantity  +=  1;
      } else {
        const itemSave = {...itemMenu, quantity: 1}
        state.items.push(itemSave);
      }

      const selectedItems = state.items.filter(item => state.items.includes(item));
...
export const getTotalSelectedItemsQuantity = (state: RootState) => state.cart.selectedTotal
